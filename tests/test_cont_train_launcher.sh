...
assert_parse() {
    ...
    local expected_warmup="${9:-4}"
    local expected_ramp_start="${10:-5}"
    local expected_ramp_end="${11:-25}"
    local expected_fixed_start="${12:-26}"
    ...
    assert_eq "$jobname protocol" "$protocol" "cont"
    assert_eq "$jobname warmup_epochs" "$warmup_epochs" "$expected_warmup"
    assert_eq "$jobname ramp_start_epoch" "$ramp_start_epoch" "$expected_ramp_start"
    assert_eq "$jobname ramp_end_epoch" "$ramp_end_epoch" "$expected_ramp_end"
    assert_eq "$jobname fixed_start_epoch" "$fixed_start_epoch" "$expected_fixed_start"
...
assert_parse \
    "linf_cont4to8_init1" \
    "convnext_small_linf_cont4to8_init1" \
    "convnext_small_linf_4_init1" \
    "linf" "madry" "pixel" "warmup_ramp_fixed" "40"
...
unset CONTINUATION_CKPT || true
parse_cont_train_job "l2_cont4to8_init1" 49140 "$tmp_root"
...
parse_cont_train_job "l2_cont4to8_init1" 49140 "/does/not/matter"
assert_eq "explicit checkpoint" "$CONTINUATION_CKPT" "$tmp_ckpt"
rm -f "$tmp_ckpt"
unset CONTINUATION_CKPT || true

if parse_cont_train_job "v1noise_linf_cont4to8_init1" 49140 "/tmp" >/dev/null 2>&1; then
