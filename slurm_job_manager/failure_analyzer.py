...
When a job fails, extract the traceback, normalize away variable tokens
(timestamps, paths, addresses, ids), and hash the structural remainder. A given
signature is escalated to the LLM analyzer + email **at most once, ever** (dedup
via ``failure_hashes``). Repeat failures with the same root cause are silently
skipped -- the bound that keeps the LLM step from looping or spamming.
...
logger = logging.getLogger("sjm.failure")
...
def analyze_failure(
    ...
    db.mark_failed(model_id, error_hash=err_hash)

    if db.has_failure_hash(err_hash):
        logger.info("failure %s already seen; skipping LLM/email for %s", err_hash, model_id)
    ...
    if emailer is not None:
        try:
            emailer(f"[sjm] new failure signature {err_hash[:8]} ({model_id})", report_md)
        except Exception as exc:
            logger.exception("emailer failed for %s: %s", model_id, exc)

    db.record_failure_hash(err_hash, model_id, report_path)
    logger.info("recorded NEW failure signature %s for %s -> %s", err_hash, model_id, report_path)
